...
class PurchaseOrderController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        // $products = PurchaseOrderProduct::select('purchase_order_id','product_id','pack_of','quantity_in_pack','quantity','hsn_code')->selectRaw("DATE_FORMAT(created_at,'%d %M %Y') AS order_date")->with('product:id,title,unique_id','purchase_orders:id,order_no,type')->orWhereHas('purchase_orders',function($q){
        //     $q->where('status', 1);
        // })->get();
