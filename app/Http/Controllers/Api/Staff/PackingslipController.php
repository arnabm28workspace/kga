...
class PackingslipController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $products = PackingslipProduct::select('packingslip_id','product_id','quantity')->selectRaw("DATE_FORMAT(created_at,'%d %M %Y') AS order_date")->with('product:id,title,unique_id,type')->with(['packingslip' => function($queryps){
            $queryps->select('id','sales_order_id','slipno','goods_out_type')->with('sales_order:id,order_no');
        }])
        ->whereHas('packingslip',function($q){
            $q->where('is_goods_out', 0);
        })
        ->get();

        return Response::json([
            'status' => true,
            'message' => "Disbursable Product List",
            'data' => array(
